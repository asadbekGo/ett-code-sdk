...
func GetUserAccountByAccessToken(widgetObject map[string]interface{}, accessToken, secretKey string, ettUcodeApi *sdk.ObjectFunction) (userAccountObject map[string]interface{}, errorResponse sdk.ResponseError) {
	...
	} else if userAccount.Phone != "" {
		orFilter = append(orFilter, map[string]interface{}{"phone_number": userAccount.Phone, "agent_contracts_id": agentContractGuid, "external_user_id": userAccount.ExternalUserId})
	}

	if len(orFilter) <= 0 {
		errorResponse.StatusCode = 401
		errorResponse.ClientErrorMessage = sdk.ErrorCodeWithMessage[errorResponse.StatusCode]
		errorResponse.ErrorMessage = ettUcodeApi.Logger.ErrorLog.Sprint("Email or Phone is required")
		return userAccountObject, errorResponse
	}

	var getUserAccountRequest = sdk.Request{Data: map[string]interface{}{"match": map[string]interface{}{"$or": orFilter}}}
	userAccountResponse, response, err := ettUcodeApi.GetListAggregate(&sdk.Argument{TableSlug: "user_account", Request: getUserAccountRequest, BlockCached: true})
	if err != nil {
		errorResponse.StatusCode = 500
		errorResponse.Description = response.Data["description"]
		errorResponse.ClientErrorMessage = sdk.ErrorCodeWithMessage[errorResponse.StatusCode]
	...
	if len(userAccountResponse.Data.Data.Response) <= 0 {
